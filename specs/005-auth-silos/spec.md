...
| OQ-011 | **Mixed Policy/Silo Enforce precedence**: When both a directly-assigned policy and a silo policy exist for an account, which `Enforce` value takes precedence? | **Open** — requires lab validation (VAL-A1-01 from research); affects enforcement flip sequencing | Blocks enforcement flip design if accounts have both direct and silo policies |
...
| OQ-013 | **RDP (NLA/CredSSP) to Tier 0 targets denied while interactive logon succeeds**: Lab-observed 2026-08-25 — with Tier 0 enforced, `t0-admin1` (Domain Admin) logs on **interactively** to DC01 and TIERLAB-SERVER (both approved origins: `Domain Controllers` / `Tier0MemberServers`), but **RDP** to either is **denied — event 105, blank Device Name**. The CredSSP-delegated fresh AS on the target does not appear to carry the target's armored device identity the way an interactive logon does. | **Open — active investigation (Day 2)**; research BRK-17/BRK-18 flagged RDP/CredSSP as validation-required. Verify target client-armoring is active (reboot + client-armoring GPO linked to the Domain Controllers OU **and** the Tier 0 Member Servers OU); investigate RestrictedAdmin RDP mode and how the RDP logon's AS is (or isn't) armored. | **Blocks enforcement** for admins who RDP to Tier 0 servers/DCs — a core Tier 0 admin path. Must be resolved before Tier 0 can be safely enforced. |
