...
- **Permanent unresolvable `gpoStatus` drift between deploy and audit**: `New-TierModelGpo.ps1:171–179` handled only 4 of the 6 valid `gpoStatus` values while `Test-TierModelGPO.ps1` handled all 6. `AllEnabled` and `AllSettingsDisabled` fell through the `default` and deployed as flags `0`, but audited as a different expected value — and because re-running deploy kept writing `0`, the drift could never converge. Both sides now map all six values identically (`AllEnabled`/`AllSettingsEnabled` → 0, `UserSettingsDisabled` → 1, `ComputerSettingsDisabled` → 2, `BothSettingsDisabled`/`AllSettingsDisabled` → 3) and each carries a comment requiring them to stay in agreement. Latent in practice: the shipped `config/tiermodel-gpos.json` uses only two of the six values. Note that deploy still retains `default { 0 }` for an *unrecognised* value (`New-TierModelGpo.ps1:178`) whereas audit now fails loudly — that remaining asymmetry is open by deliberate owner decision and tracked as **BUG-016**; it is not addressed in this release.
...
### Added
- **`gpoStatus` enum in `config/tiermodel.schema.json:72`** listing the six valid values. This is an **editor / IntelliSense convenience only** — nothing in the product performs JSON-Schema validation at runtime. `Test-TierModelConfig` is referenced only from `tests/`, its hand-rolled checker reads only `required` plus a semver regex and ignores `enum`, `type` and `additionalProperties`, and `Test-Json` appears nowhere in the repository. Runtime enforcement of `gpoStatus` comes from the deploy/audit lookup tables described above, not from the schema.

### Tests
- **Corrected a mock that produced two phantom integration failures against correct production code**: the local `Set-GPRegistryValue` mock in `tests/Integration.WinLapsDeployment.Tests.ps1:122–126` returned nothing, but the real cmdlet — verified against a live domain controller, where it is a Script Function proxy with no `-PassThru` — returns a non-null PSObject. The mock now returns a `[PSCustomObject]`, so the new write-verification result checks behave as they do in production. This corrects the local mock only — the shared stubs in `tests/helpers/ADStubs.ps1:94` and `:106` still declare `Set-GPRegistryValue` with an empty body and remain open as **BUG-015**.
